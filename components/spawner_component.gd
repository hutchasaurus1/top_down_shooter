...
func spawn(global_spawn_position: Vector2 = global_position, parent: Node = get_tree().current_scene, rotation_angle: float = 0) -> Node:
	assert(scene is PackedScene, "Error: The scene export was never set on this spawner component.")
	# Instance the scene
	var instance = scene.instantiate()
	# Add it as a child of the parent
	call_deferred("create_child", instance, parent)
...
func create_child(instance, parent: Node = get_tree().current_scene):
